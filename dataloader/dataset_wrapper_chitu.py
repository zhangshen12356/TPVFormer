...
class DatasetWrapper_Chitu(data.Dataset):
    def __init__(self, in_dataset, grid_size, num_classes,
    ...
    def __getitem__(self, index):
        ...
        cur_grid_size = self.grid_size                 # 200, 200, 16  self.grid_size [100, 100, 8]
        ...
        # process labels
        processed_label = np.ones(self.grid_size, dtype=np.uint8) * self.num_classes  # self.fill_label=17 (100,100, 8)
...
def custom_collate_fn_chitu(data):
    img2stack = np.stack([d[0] for d in data]).astype(np.float32)
    meta2stack = [d[1] for d in data]
    label2stack = np.stack([d[2] for d in data]).astype(np.int)
    grid_ind_stack = [d[3] for d in data]
    point_label = [d[4] for d in data]
    return torch.from_numpy(img2stack), meta2stack, torch.from_numpy(label2stack), grid_ind_stack, point_label