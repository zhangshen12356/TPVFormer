...
class ImagePoint_NuScenes(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        dict_keys(['lidar_path', 'token', 'sweeps', 'cams', 'lidar2ego_translation', 'lidar2ego_rotation', 
            'ego2global_translation', 'ego2global_rotation', 'timestamp', 'gt_boxes', 'gt_names', 'gt_velocity', 
            'num_lidar_pts', 'num_radar_pts'])
        '''
        imgs_info = self.get_data_info(info)
        '''
        dict_keys(['sample_idx', 'pts_filename', 'sweeps', 'ego2global_translation', 'ego2global_rotation', 
        ...
        '''
        dict_keys(['lidar2img'])
        len(img_metas['lidar2img'])=6, img_metas['lidar2img'][0].shape:(4, 4)
        '''
        # read 6 cams
        imgs = []
        ...
        '''
        len(imgs)=6, imgs[0].shape: (900, 1600, 3)
        '''
        ...
        points_label = np.vectorize(self.learning_map.__getitem__)(points_label)  # (34720, 1)每一个点云的标签,numpy类型

        lidar_path = info['lidar_path']
        points = np.fromfile(lidar_path, dtype=np.float32, count=-1).reshape([-1, 5]) # (34752, 5)
