...
if __name__ == "__main__":
    ...
    for index in args.frame_idx:
        ...
        with torch.no_grad():
            ...
            # -----------------------save file-------------------------------------------------------
            # lidar2img_save = np.array(lidar2img)[0].astype(np.float32).tofile("/home/zs/Code/TPVFormer_new/lidar2img_6.bin")
            # img_shape_save = np.array(img_shape).astype(np.float32).tofile("/home/zs/Code/TPVFormer_new/img_shape_6.bin")
            # points_save = grid.clone().cpu().numpy()[0].astype(np.float32).tofile("/home/zs/Code/TPVFormer_new/points_6.bin")
            # ----------------------------------------------------------------------------------------
            lidar2img = torch.from_numpy(np.array(lidar2img).astype(np.float32)).to(device)
            img_shape = torch.from_numpy(np.array([img_shape]).astype(np.float32)).to(device)

            my_model.forward = my_model.forward_trt
            # my_model(imgs, grid.clone(), lidar2img, img_shape)
            points = False
            if points:
                torch.onnx.export(my_model, (imgs, grid.clone(), lidar2img, img_shape),
                                ...
                                dynamic_axes={"imgs": {0: "batch_size"}, "points": {0: "batch_size", 1: "num_points"}, "lidar2img": {
                                    0: "batch_size"}, "img_shape": {0: "batch_size"}},
                                autograd_inlining=False)
                print("TPVFormer Convert to ONNX Done!!!")
            
            # ----------------------------------------------------infer without points----------------------------------------------
            else:
                my_model.forward = my_model.forward_trt_no_points
                torch.onnx.export(my_model, (imgs, lidar2img, img_shape),
                                "/home/zs/Code/TPVFormer_new/tpvformer_no_points.onnx",
                                verbose=True,
                                opset_version=17,
                                input_names=["imgs", "lidar2img", "img_shape"],
                                output_names=["output1"],
                                dynamic_axes={"imgs": {0: "batch_size"}, "lidar2img": {
                                    0: "batch_size"}, "img_shape": {0: "batch_size"}},
                                autograd_inlining=False)
                print("TPVFormer without points Convert to ONNX Done!!!")

           
            
# pip install ONNX-Simplifier
# python -m onnxsim tpvformer.onnx tpvformer_sim.onnx
