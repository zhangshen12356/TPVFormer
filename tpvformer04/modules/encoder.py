...
class TPVFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(self, *args, tpv_h, tpv_w, tpv_z, pc_range=None, 
                 num_points_in_pillar=[4, 32, 32], bs=1, return_intermediate=False, 
                 **kwargs):
        ...
        ref_3d_hw = self.get_reference_points(tpv_h, tpv_w, pc_range[5]-pc_range[2], num_points_in_pillar[0], '3d', bs, device='cpu')

        ref_3d_zh = self.get_reference_points(tpv_z, tpv_h, pc_range[3]-pc_range[0], num_points_in_pillar[1], '3d', bs, device='cpu')
        ref_3d_zh = ref_3d_zh.permute(3, 0, 1, 2)[[2, 0, 1]]
        ref_3d_zh = ref_3d_zh.permute(1, 2, 3, 0)

        ref_3d_wz = self.get_reference_points(tpv_w, tpv_z, pc_range[4]-pc_range[1], num_points_in_pillar[2], '3d', bs, device='cpu')
        ...
        ref_2d_hw = self.get_reference_points(tpv_h, tpv_w, dim='2d', bs=bs, device='cpu')
        ref_2d_zh = self.get_reference_points(tpv_z, tpv_h, dim='2d', bs=bs, device='cpu')
        ref_2d_wz = self.get_reference_points(tpv_w, tpv_z, dim='2d', bs=bs, device='cpu')
