...
class TPVCrossViewHybridAttention(BaseModule):
    ...
    def forward(self,
        ...
        if value is None:
            value = torch.cat([query, query], 0)  # 后面需要value的x和y方向上进行采样，所以将两个同样的进行cat
        ...
        query = torch.cat([value[:int(bs)], query], -1)
        ...
        '''
        位置偏移量是由query经过全连接得到的
        self.sampling_offsets = nn.Linear(embed_dims * num_tpv_queue, num_tpv_queue * num_heads * num_levels * num_points * 2)
        Linear(in_features=256*2, out_features=2*8*1*4*2, bias=True)
        thetas ，多头不同的方向
        '''
        ...
        '''
            self.attention_weights = nn.Linear(embed_dims * num_tpv_queue, num_tpv_queue * num_heads * num_levels * num_points)
            Linear(in_features=256*2, out_features=2*8*1*4, bias=True)
            预测采样点注意力权重
        '''
        attention_weights = attention_weights.softmax(-1)  # 对权重进行归一化
        ...
        attention_weights = attention_weights.reshape(bs*self.num_tpv_queue, num_query, self.num_heads, self.num_levels, self.num_points).contiguous()
        sampling_offsets = sampling_offsets.permute(3, 0, 1, 2, 4, 5, 6)
        sampling_offsets = sampling_offsets.reshape(bs*self.num_tpv_queue, num_query, self.num_heads, self.num_levels, self.num_points, 2)

        if reference_points.shape[-1] == 2:  # 2表示w和h
