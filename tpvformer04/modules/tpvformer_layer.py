        ...
        for layer in self.operation_order:
            ...
            if layer == 'self_attn':
                query_0 = self.attentions[attn_index](
                    query[0],  # torch.Size([1, 10000, 256])
                    None,
                    None,
                    identity if self.pre_norm else None,
                    query_pos=tpv_pos[0],     # torch.Size([1, 10000, 256])
                    reference_points=ref_2d,  # torch.Size([2, 10000, 1, 2])
                    spatial_shapes=torch.tensor(
                        [[tpv_h, tpv_w]], device=query[0].device),  # torch.Size([1, 2])
                    level_start_index=torch.tensor([0], device=query[0].device), # torch.Size([1])
                    **kwargs)
                attn_index += 1
            ...
            # image cross attention
                ...
                query = self.attentions[attn_index](
                    query,  # torch.Size([1, 11600, 256])
                    key,    # torch.Size([6, 30825, 1, 256])
                    value,  # torch.Size([6, 30825, 1, 256])
                    # identity if self.pre_norm else None,
                    # len(reference_points_cams)=3 torch.Size([6, 1, 10000, 4, 2]), torch.Size([6, 1, 800, 32, 2]), torch.Size([6, 1, 800, 32, 2])
                    reference_points_cams=reference_points_cams,
                    # len(tpv_masks)=3 torch.Size([6, 1, 10000, 4]), torch.Size([6, 1, 800, 32]), torch.Size([6, 1, 800, 32])
                    tpv_masks=tpv_masks,
